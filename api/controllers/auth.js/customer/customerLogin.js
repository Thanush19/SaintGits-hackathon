...
const customerLogin = async (req, res) => {
    ...
    try {
        ...
        const search_query = {
            text : "SELECT * FROM customers WHERE aadhaar_number = $1",
        ...
        if(result.rows.length === 0) {
            ...
            if(phone_number === user.phone_number) {
                ...
                const token = jwt.sign({ category : "customer", user : user.id }, "SECRET_KEY");

                res.status(200).json({token, user, category : "customer"});
